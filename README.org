...
For analyzing Java, MetaDL need access to a Java 7 runtime library (you can download it [[https://www.oracle.com/java/technologies/javase/javase7-archive-downloads.html#license-lightbox][here]]). Set ~METADL_JAVA_RT=/path/to/jre/lib/rt.jr~ in your environment to let MetaDL know where to find this library.
...
- ~IMPORT('P, "program.java", "java8")~ generate a program representation relation
from ~program.java~ Java 8 program
